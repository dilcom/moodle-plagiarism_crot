...
$soap_location = $plagiarismsettings['crot_soap_service_location'];
...
if (!empty($files)){
    foreach ($files as $afile){
      ...
      //  $docid = mysql_insert_id();
		...
		$atext=mb_strtolower($btext, "utf-8");
    ...
    // store fingerprint
        ...
        if ($plagiarismvalues['crot_local']==1) {
			...
			// comparing fingerprints and updating plagiarism_crot_sub_pair table
			...
			foreach ($pair_submissions as $pair_submission){
				...
				if ($webdoc = $DB->get_record("plagiarism_crot_web_docs", array('document_id'=>$pair_submission->id)))
					continue;
				...
				if ($docid!=$pair_submission->id){
					...
					$sql_query ="SELECT * FROM {$CFG->prefix}plagiarism_crot_sub_pair 
					...
					if (! $pair){
						...
						$DB->insert_record("plagiarism_crot_sub_pair", $pair_record);	
					} else {
						// TODO update		
					}
        ...
        if ($plagiarismvalues['crot_global']==1) {
			...
			$wdocid = $DB->insert_record("plagiarism_crot_documents", $wdocrecord);
               ...
               // mysql_query($query);
				...
				$webdocid = $DB->insert_record("plagiarism_crot_web_docs", $webdocrecord);
                //$weblink = urlencode($manUrl->mainUrl);
                //$weblinklive = urlencode($manUrl->msUrl);
                //$query = "INSERT INTO {$CFG->prefix}plagiarism_crot_web_docs (document_id,link,link_live,is_from_cache,related_doc_id) VALUES ('$wdocid','$weblink','$weblinklive','false','$docid')";			
                //mysql_query($query);
				...
				if(!is_null($similarnumber->cnt) && $similarnumber->cnt!=0 ){
					...
					$pair_record->number_of_same_hashes = $similarnumber->cnt;
					$ppair = $DB->insert_record("plagiarism_crot_sub_pair", $pair_record);
				} else {
		...
		if ($plagiarismvalues['crot_shingle']==1) {
			echo "Starting search in cathedral dataBase\n";
			try {
        			$client = new SoapClient($soap_location);
				$req = new t__text();
				$req->authorGroup = "moodle_crot";
				$req->authorName = "moodle";
				$req->name = $filename;
				$req->type = 0;
				$req->streamData = mb_convert_encoding($btext, "UTF-8", "auto");
				$req->similarity = 0.0;
				$res = $client->CompareText($req);
			}
			catch (exception $e){
				echo "exception in SOAP connection!\n";
				echo $e;
			}
			$res = $res->arrayOfTexts;
			if (!is_array($res))
				$res = array($res);
			if ($res[0] != NULL)
				foreach ($res as $result) {
					$result = mb_ereg_replace('#\s{2,}#',' ',$result->streamData); 
					...
					// insert doc into web_doc table
					$webdocrecord->document_id = $wdocid;
					$webdocrecord->link=urlencode("none");
					$webdocrecord->link_live=urlencode("none");
					$webdocrecord->is_from_cache=false;
					$webdocrecord->related_doc_id = $docid;
					$webdocid = $DB->insert_record("plagiarism_crot_web_docs", $webdocrecord);
					//$weblink = urlencode($manUrl->mainUrl);
