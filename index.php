...
//sw 08/28 end
    ...
    $sql_query = "SELECT * FROM {$CFG->prefix}plagiarism_crot_sub_pair WHERE submission_a_id ='$ida' OR  submission_b_id = '$ida' order by number_of_same_hashes desc";
	...
	if (!empty($similars)){
        foreach ($similars as $asim){
			...
			// web document
                $wwwdoc = $DB->get_record("plagiarism_crot_web_docs", array("document_id"=>$party));					
